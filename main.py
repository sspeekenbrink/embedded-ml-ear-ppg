...
from ppg_pipeline.data import find_data_sources, create_dataset_from_sources, PPGWindowDataset
from ppg_pipeline.models import PPGPeakDetectorCNN, DilatedCNN
from ppg_pipeline.training import run_single_training, run_grid_search, run_pareto_search, FocalLoss
...
def main():
    ...
    args_dict = vars(args)
    args_for_log = {k: str(v) if isinstance(v, Path) else v for k, v in args_dict.items()}
    ...
    if args.plot_nms_comparison and args.nms == 0:
        logging.warning("--plot_nms_comparison requires --nms to be set (e.g., --nms 5). Setting --nms to 5.")
        args.nms = 5

    logging.info(f"Scanning for data sources in '{args.data_dir}'...")
    sources = find_data_sources(args.data_dir)
    if not sources:
        logging.error("No valid data sources found. Please check the --data_dir path and folder structure.")
        sys.exit(1)
    ...
    logging.info(f"Successfully created a combined dataset with {len(dataset)} windows of size {windows.shape[1]}.")
    ...
    if args.grid_search:
        if args.pareto_front:
            ...
            _, model, trials = run_grid_search(train_loader, test_loader, criterion, args)
            training_metrics = {"search_type": "grid_search", "trials": trials}
    else:
        model = DilatedCNN().to(args.device)
        optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr)
        model, tuned_threshold, y_true, y_pred, single_metrics = run_single_training(model, train_loader, test_loader, criterion, optimizer, args)
    ...
    if args.plot_nms_comparison and model is not None:
        ...
        focus_mode = "focusing on windows with most NMS removals" if args.focus_nms_removed else "random windows"
    ...
    if training_metrics is not None:
        ...
        try:
            ...
            logging.error(f"Failed to save training metrics to {args.save_metrics_json}: {e}")
