...
def get_argument_parser():
    ...
    p.add_argument("--window_s", type=float, default=4.0, help="Window size in seconds.")
    p.add_argument("--shift_s", type=float, default=1.0, help="Shift between consecutive windows in seconds.")
    ...
    p.add_argument("--train_split", type=float, default=0.8, help="Fraction of data to use for training.")
    p.add_argument("--epochs", type=int, default=25, help="Number of epochs for single model training.")
    p.add_argument("--batch_size", type=int, default=64, help="Batch size for training and evaluation.")
    p.add_argument("--lr", type=float, default=1e-3, help="Learning rate for the AdamW optimiser.")
    p.add_argument("--device", default="cuda" if torch.cuda.is_available() else "cpu",
                   help="Device to use for training (cuda or cpu).")
    p.add_argument("--loss", choices=["bce", "focal"], default="bce", help="Loss function to use ('bce' or 'focal').")
    p.add_argument("--sample_balance", action="store_true",
                   help="Use WeightedRandomSampler to balance windows with/without peaks.")

    p.add_argument("--grid_search", action="store_true", help="Run a grid search over CNN hyperparameters.")
    p.add_argument("--pareto_front", action="store_true",
                   help="Run a Pareto front search over CNN hyperparameters (requires --grid_search).")
    p.add_argument("--epochs_per_trial", type=int, default=8, help="Epochs per configuration during grid search.")
    p.add_argument("--max_model_size", type=int, default=None,
                   help="Maximum model size in Kbytes for Pareto front search.")
    p.add_argument("--save_top_models", action="store_true", help="Save Pareto-optimal models for later use.")
    p.add_argument("--top_model_dir", type=Path, default=Path("top_models"),
    ...
    p.add_argument("--plot_nms_comparison", action="store_true",
                   help="Compare predictions with and without NMS. Overrides normal workflow.")
    p.add_argument("--focus_nms_removed", action="store_true",
                   help="Focus on windows where NMS removes the most points (requires --plot_nms_comparison).")
    p.add_argument("--model_path", type=Path, default=Path("models/model_00.pt"),
...
def parse_cli_args():
    ...
    if args.downsample is not None and args.downsample >= args.target_fs:
        print("[Warn] --downsample >= --target_fs; final rate will be the same or higher than the common rate.")

    if args.decision_threshold is not None:
        args.decision_threshold = float(max(1e-6, min(1.0 - 1e-6, args.decision_threshold)))
    ...
    if (args.bandpass_low is not None) != (args.bandpass_high is not None):
        parser.error("Both --bandpass_low and --bandpass_high must be provided together.")
