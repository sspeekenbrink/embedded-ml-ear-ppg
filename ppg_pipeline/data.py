...
def find_data_sources(root_dir: Path) -> List[DataSource]:
    ...
    for subject_dir in root_dir.iterdir():
        ...
        try:
            ...
            logging.info(f"Successfully found and parsed data source: '{subject_dir.name}'")
...
def downsample_ppg(ppg_signal: np.ndarray, original_fs: float, target_fs: float) -> Tuple[np.ndarray, float]:
    """
    ...
    logging.info(f"Resampled PPG from {original_fs:.6f} Hz to {new_fs:.6f} Hz (len {len(ppg_signal)} → {len(resampled)})")
...
def _load_snapped_peaks_if_available(subject_dir: Path, *, ignore_peak_json: bool) -> np.ndarray | None:
    """
...
def create_ppg_windows(
    ...
    def window_overlaps_excluded_range(window_start: int, window_end: int) -> bool:
        ...
        for exclude_range in relative_exclude_ranges:
            if window_start < exclude_range['end'] and window_end > exclude_range['start']:
                return True
        return False
...
def resample_to_rate(signal: np.ndarray,
                     original_fs: float,
                     target_fs: float,
                     *,
                     max_denominator: int = 1000) -> Tuple[np.ndarray, float]:
    """
...
def create_dataset_from_sources(
    ...
    logging.info(f"[Dataset] target_fs={target_fs:.6f} Hz"
                 + (f", final downsample={args.downsample:.6f} Hz" if args.downsample else ""))

    for source in sources:
        logging.info(f"Processing source: {source.id} (ppg_fs={source.ppg_fs:.6f}, ecg_fs={source.ecg_fs:.6f})")

        # Load raw
        ppg_raw = SensorData(file_location=str(source.ppg_path), sample_rate=source.ppg_fs).led_green_values
        df_ecg  = pd.read_csv(source.ecg_path)
        ecg_raw, ecg_t = df_ecg["channel"].values, df_ecg["time"].values

        current_ppg, current_ppg_fs = resample_to_rate(ppg_raw, source.ppg_fs, target_fs)

        if args.downsample:
            current_ppg, current_ppg_fs = downsample_ppg(current_ppg, current_ppg_fs, float(args.downsample))

        if hasattr(args, 'bandpass_low') and hasattr(args, 'bandpass_high') and args.bandpass_low is not None and args.bandpass_high is not None:
            logging.info(f"{source.id}: applying bandpass filter ({args.bandpass_low:.2f} - {args.bandpass_high:.2f} Hz) to PPG signal")
            current_ppg = bandpass_filter(current_ppg, current_ppg_fs, args.bandpass_low, args.bandpass_high)

        snapped_times_s = _load_snapped_peaks_if_available(source.ppg_path.parent, ignore_peak_json=bool(getattr(args, 'ignore_peak_json', False)))

        if snapped_times_s is not None:
            rpeaks_idx = np.round(snapped_times_s * current_ppg_fs).astype(int)
            logging.info(f"{source.id}: using {len(rpeaks_idx)} peaks from peaks_snapped.json")
        else:
            ...
            if snap_w is not None:
                ...
                logging.info(f"{source.id}: snapped peaks within ±{float(snap_w):.3f}s window -> {len(rpeaks_idx)} peaks")

        if source.exclude_ranges:
            logging.info(f"{source.id}: excluding {len(source.exclude_ranges)} time ranges from windowing")

            windows, labels = create_ppg_windows(
            current_ppg, rpeaks_idx, current_ppg_fs, args.window_s, args.shift_s, source.exclude_ranges
        )
        ...
        if windows.size > 0 and windows.shape[1] != expected_len:
            ...
            logging.warning(f"[Dataset] {source.id}: adjusted window length {wl}→{expected_len}")

        if windows.size > 0:
            all_windows.append(windows.astype(np.float32, copy=False))
            all_labels.append(labels.astype(np.int64, copy=False))
            logging.info(f"-> {source.id}: {len(windows)} windows, shape {windows.shape} at {current_ppg_fs:.6f} Hz")
        else:
            logging.warning(f"-> No windows from {source.id}. Check signal quality/alignment.")
    ...
    logging.info(f"[Dataset] Final windows {final_windows.shape}, labels {final_labels.shape}")
